...
class ConfigActivity : BaseActivity() {
    ...
    private fun initView() {
        ...
        configBinding.etWtUrl.setText(PreferenceUtils.getInstance().wtUrl)
...
//        configBinding.etAircraftHeading.setText(PreferenceUtils.getInstance().aircraftHeading)
    ...
    private fun config() {
        ...
        if (TextUtils.isEmpty(configBinding.etWtUrl.text)) {
            ToastUtil.showToast("未配置风机巡检文件上传Url")
            return
        }
        ...
        if (configBinding.rbRtmp.isChecked) {
            if (TextUtils.isEmpty(configBinding.etStreamUrl.text)) {
                ToastUtil.showToast("未配置Rtmp推流地址")
                return
            }
        }
...
//        }
        ...
        PreferenceUtils.getInstance().wtUrl =
            configBinding.etWtUrl.text.toString().replace(" ", "")
